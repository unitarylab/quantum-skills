...
### Exercise 1: Basic Amplitude Estimation (Beginner)

**Goal**: Use the provided implementation to estimate success probabilities.

```python
# TODO: Create three different state preparations:
# 1. A 2-qubit uniform superposition (p=0.25)
# 2. A 3-qubit uniform superposition (p=0.125)
# 3. A weighted superposition with p=0.5

# For each, run amplitude estimation with d=6 and verify the result
# is close to the theoretical value
```

**Verification**: Estimated values should be within ~0.01 of theoretical values.

---

### Exercise 2: Precision vs. Precision Trade-off (Intermediate)

**Goal**: Understand how precision parameter $d$ affects accuracy.

```python
# TODO: For a fixed success probability (e.g., p=0.36):
# 1. Run amplitude estimation with d=4, 5, 6, 7, 8
# 2. For each, record: estimated_p, histogram_peak_prob
# 3. Plot: How does peak probability change with d?
#          How does estimation error decrease with d?

# Expected: Higher d → narrower distribution but same peak height
```

**Analysis**: Why does increasing $d$ not improve the peak probability?

---

### Exercise 3: Multi-Qubit Oracle Design (Intermediate)

**Goal**: Create a custom "good" state predicate.

```python
# TODO: Design a 4-qubit superposition where:
# - Good state: qubits 0,2 are |0⟩ (qubits 1,3 can be anything)
# - Run amplitude estimation and verify results

# Hint: What is the theoretical success probability immediately
# after state preparation with H gates?
```

**Challenge**: Verify your theoretical calculation matches the measured value.

---

### Exercise 4: Custom Implementation (Advanced)

**Goal**: Implement amplitude estimation from scratch.

```python
# TODO: Implement your own AmplitudeEstimation class with:
# 1. Grover operator construction
# 2. QPE circuit building
# 3. Inverse QFT implementation
# 4. Phase histogram extraction
# 5. Phase-to-amplitude conversion

# Verify: Results match the provided implementation
```

---

### Exercise 5: Application Challenge (Expert)

**Goal**: Apply amplitude estimation to a Monte Carlo simulation.

```python
# TODO: Estimate the probability of a random event:
# - Create a quantum circuit that encodes the event
# - Use amplitude estimation to estimate P(event)
# - Compare with classical Monte Carlo
# - What quantum advantage do you observe?

# Example: Estimating π using random point sampling in a circle
```
...
- **Mathematical Foundation**: See [README.md](README.md) for complete mathematical derivations
- **Source Implementation**: Study [algorithm.py](algorithm.py) for production-quality code
- **Related Algorithms**:
  - [Amplitude Amplification](../amplitude-amplification/SKILL.md): Foundation for this algorithm
  - [Quantum Phase Estimation](../phase-estimation/SKILL.md): The QPE component
  - [Grover's Algorithm](../grovers-algorithm/SKILL.md): The search component
- **Applications**: Monte Carlo simulations, option pricing, optimization problems
- **Papers**: Original Brassard et al. (2000) amplitude estimation paper
...
- [ ] Explain why amplitude estimation has quadratic speedup
- [ ] Understand how Grover operator encodes amplitude as phase
- [ ] Describe the role of QPE in amplitude estimation
- [ ] Use `AmplitudeEstimationAlgorithm` correctly
- [ ] Interpret phase histograms and select the best phase
- [ ] Convert measured phase to success probability
- [ ] Design custom oracle for arbitrary good states
- [ ] Implement amplitude estimation from scratch
- [ ] Apply to amplitude amplification circuits
- [ ] Estimate success probabilities to desired accuracy
- [ ] Analyze precision-accuracy tradeoffs
...
Amplitude estimation can enhance Monte Carlo algorithms by providing quadratic speedup for estimating probabilities.

### Option Pricing (Finance)
Estimate the probability of profitable outcomes in financial derivatives.

### Combinatorial Optimization
Estimate the fraction of solutions satisfying specific constraints.

### Machine Learning
Estimate success probabilities in quantum machine learning algorithms.

---

*Ready to master quantum amplitude estimation? Perfect your understanding step by step, from theory to practice!* 🚀
