...
- **Prerequisites**:
  - [Quantum Phase Estimation](../primitives/quantum-phase-estimation/SKILL.md)
  - [Amplitude Amplification](../amplitude-amplification/SKILL.md)
- **Advanced Extensions**:
  - **Variational HHL**: Hybrid classical-quantum for large systems
  - **Iterative Methods**: Combine with classical iterative refinement
  - **Error Mitigation**: Reduce hardware noise effects
...
- [ ] Explain quantum advantage of HHL vs classical solvers
- [ ] Describe the five steps of HHL algorithm
- [ ] Understand role of QPE in eigenvalue extraction
- [ ] Explain the controlled reciprocal rotation (non-linear step)
- [ ] Calculate optimal evolution time $t$
- [ ] Understand post-selection success probability  $O(1/\kappa^2)$
- [ ] Apply block embedding for non-Hermitian matrices
- [ ] Use `HHLAlgorithm` class for various problem sizes
- [ ] Interpret condition number and its impact
- [ ] Handle Hermitian constraints properly
- [ ] Implement HHL from scratch
- [ ] Apply amplitude amplification for improved success rate
- [ ] Connect to practical applications (PDEs, ML, etc.)
...
Linear regression: Solve $A\mathbf{x} = \mathbf{b}$ where $A$ is the design matrix
- Classical: $O(N^3)$ or $O(Ns\kappa\log(1/\epsilon))$ iterative
- HHL (if sparse, well-conditioned): $O(\log N s^2 \kappa^2 / \epsilon)$
-  **Exponential advantage for recommendation systems, kernel methods**

### Scientific Computing
Solving Poisson equation $\nabla^2 u = f$ discretized to $Au = b$
- Finite difference/element creates sparse tridiagonal $A$
- HHL leverages sparsity for speedup
- **Can solve problems impossible classically**

---

*Mastering HHL unlocks quantum computational advantage for one of the most fundamental problems in scientific computing!* 🚀
