...
Discretize PDE $\nabla^2 u = f$ as matrix equation $Au = f$:
- Create block-encoding of discrete Laplacian A
- Run QSVT solver
- Extract solution coefficients from quantum state
- Quadratic speedup in matrix dimension $N$

### Quantum Machine Learning - Linear Regression

Solve normal equations: $(X^T X)w = X^T y$
- Prepare block-encoding of Gram matrix $X^T X$
- Use QSVT to compute $(X^T X)^{-1}$
- Benefits from low condition number after preprocessing

### Eigenvalue/Singular Value Problems

For computing eigenvalues $\lambda_i$ of Hermitian $H$:
- Use QSVT with Chebyshev polynomial $T_n(H/λ_{\max})$
- Extract eigenvalues from amplitudes
- Polynomial-time verification (no measurement collapse)
